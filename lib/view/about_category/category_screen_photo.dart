...
import 'package:planner/view_model/comment_view_model.dart';
...
class _CategoryScreenPhotoState extends State<CategoryScreenPhoto> {
  ...
  Widget build(BuildContext context) {
    ...
    final commentAudioViewModel =
        Provider.of<CommentAudioViewModel>(context, listen: false);
            ...
            child: StreamBuilder<List<PhotoModel>>(
              ...
              builder: (context, photosSnapshot) {
                ...
                if (photosSnapshot.hasError) {
                  return Center(
                    child: Text('Error: ${photosSnapshot.error}'),
                  );
                }
                ...
                return Column(
                  children: [
                    // 영상 만들기 버튼, 카테고리 추천 버튼
                    _buildActionButtons(context, categoryViewModel),
                    // 사진 그리드
                    _buildPhotoGrid(
                      context,
                      photos,
  ...
  /// 'recap 영상 만들기', '카테고리 추천' 두 버튼을 담은 위젯
    ...
    return Row(
      ...
      children: [
        ElevatedButton(
          ...
          onPressed: () {
            // recap 영상 생성 화면으로 이동 (VideoPlayerScreen)
            Navigator.push(
              context,
              MaterialPageRoute(
        ...
        ElevatedButton(
          ...
          // 선택된 카테고리에 대한 추천 로직 실행
          onPressed: () =>
              categoryViewModel.suggestNextCategory(widget.categoryId),
        ),
      ],
    );
  }
    ...
    showDialog(
      ...
      builder: (_) => Dialog(
        child: StreamBuilder<List<Map<String, dynamic>>>(
          stream:
              commentAudioViewModel.fetchComments(widget.categoryId, photo.id),
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          if (photo.audioUrl != null) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content: Text('Audio URL is not available')),
                            );
                          }
                        },
                      ),
